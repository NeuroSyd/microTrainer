...
class ECGSequence(Sequence):
    @classmethod
    def get_train_and_val(cls, path_to_hdf5, hdf5_dset, path_to_csv, batch_size=8, val_split=0.02, end_idx=None):
        n_samples = len(pd.read_csv(path_to_csv))
        if end_idx is None:
            n_train = math.ceil(n_samples*(1-val_split))
        else:
            n_train = math.ceil(end_idx*(1-val_split))
    ...
    def __init__(self, path_to_hdf5, hdf5_dset, path_to_csv=None, batch_size=8,
        ...
        if path_to_csv is None:
            ...
            self.y = pd.read_csv(path_to_csv).values.astype(np.float32)
        # Get tracings
